...
Thin TypeScript SDK for the current BrewMe HTTP API.
...
`contracts/source/openapi.yaml`, not re-implement BrewMe business logic in
package form.
...
- This is a **builder-facing API package**, not a replacement for the repo-local
  operator commands in `./bin/brewme`.
