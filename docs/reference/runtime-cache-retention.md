...
- repo-owned external cache/state now belongs under `SOURCE_HARBOR_CACHE_ROOT`; the legacy home-level `.brewme/` root is only a migration input, not canonical runtime storage.
...
- `.serena/cache` is a local MCP/tool cache exception. Keep it ignored, but do not fold it into BrewMe runtime-cache governance.
