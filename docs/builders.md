...
BrewMe is the builder side of the same control tower.
...
- **MCP**: the root Python package now produces the real `brewme-mcp` install artifact, the registry template points at the PyPI identifier `brewme`, and the repo now also tracks per-directory submission packet drafts; live registry/PyPI read-back still needs submit/publish proof.
...
| `ghcr.io/xiaojiou176-open/brewme-api` | dedicated API image route | product-facing API container lane; GHCR read-back now exists, but anonymous pull is still blocked until package visibility becomes public |
