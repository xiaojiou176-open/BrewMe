...
- **MCP**: the root Python package now produces the real `brewme-mcp` install artifact, the registry template points at the PyPI identifier `brewme`, and both the Official MCP Registry entry and the PyPI project already have live public read-back at `0.1.14`; the repo-controlled package line is `0.1.19`, so the remaining gap is refresh credentials, not first-time publication proof.
