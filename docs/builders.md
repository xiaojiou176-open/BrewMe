...
- **MCP**: the root Python package now produces the real `brewme-mcp` install artifact, the registry template points at the PyPI identifier `brewme`, and the PyPI project still has a live public read-back at `0.1.14`; a fresh anonymous Official MCP Registry index read-back does not currently return `BrewMe`, so the registry line should be treated as unverified until publish/refresh plus read-back exists. The repo-controlled package line is `0.1.19`.
