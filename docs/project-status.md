...
| Official MCP Registry listing + metadata template | **ship-now** | a fresh registry API read-back still confirms an active `BrewMe MCP` entry at `0.1.14`; the package line is build-ready at `0.1.19`, but refresh is currently blocked by missing publish credentials |
