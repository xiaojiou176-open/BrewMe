...
| Official MCP Registry listing + metadata template | **ship-now** | the repo ships a registry-ready metadata template and package line at `0.1.19`, but a fresh anonymous registry index read-back does not currently return `BrewMe`, so live listing truth is still unverified until publish/refresh plus read-back exists |
