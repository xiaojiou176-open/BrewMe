...
| Official MCP Registry listing + metadata template | **ship-now** | a fresh registry API read-back still confirms an active `BrewMe MCP` entry at `0.1.14`; the remaining work is version refresh, not first publication |
| PyPI package surface | **ship-now** | the public `brewme` project is still live on PyPI at `0.1.14`; the remaining work is a package-version refresh to catch back up with the repo-controlled `0.1.19` line |
...
- official-surface public distribution is now split more sharply:
  ...
  - `mcpservers.org` public search currently returns `0` servers for `brewme`
...
| MCP.so direct listing | a fresh direct page read-back at `https://mcp.so/server/brewme-mcp` currently says `Project not found` | the repo-side packet exists, but this public route does not currently prove a live listing | capture either a real submit/accept receipt or a fresh direct-page read-back showing the listing has appeared |
| mcpservers.org listing | a fresh public search for `brewme` currently returns `0` servers | the repo-side packet exists, but the public search surface still does not show a live listing | capture either the approval/listing URL or a later public search result that shows the server |
