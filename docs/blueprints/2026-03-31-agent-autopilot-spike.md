...
BrewMe already has a production-ready autopilot.
...
- a public claim that BrewMe already runs its own research loop end to end
...
| --- | --- | --- |
| Should BrewMe pursue Agent Autopilot now? | **Yes, but only as a bounded spike** | the repo already has workflows, MCP, retrieval, and evidence surfaces |
...
- MCP official docs: <https://modelcontextprotocol.io/docs/getting-started/intro>
  - BrewMe should keep using MCP as the agent-facing doorway into the
...
5. Only after approval does BrewMe execute the workflow or send the
   report.
...
- BrewMe already has Agent Autopilot
...
- BrewMe already has the primitives for an approval-first Autopilot spike
