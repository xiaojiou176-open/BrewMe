...
| `TEMPORAL_TASK_QUEUE` naming alignment | R1 | public and runtime defaults aligned on `brewme-worker` | `.env.example`, bootstrap/full-stack scripts, worker env logs | Wave 0 | Verified and ready | None |
...
| sample corpus / playground | R7 | landed with explicit sample labeling | `docs/samples/README.md`, `docs/samples/brewme-demo-corpus.json`, `/playground` | Wave 3 | Verified and ready | keep sample/live boundary explicit |
...
| Item | Archive source | Current repo status | Evidence | Wave | Final classification | Remaining blocker / next step |
| --- | --- | --- | --- | --- | --- | --- |
| keep `BrewMe` as product name, use `brewme.ai` only as marketing domain if needed | R6 | current repo follows this recommendation | README, project-positioning, GitHub profile intent | Waves 3-5 | Deferred by design | only revisit when there is a real hosted story |
