...
- `publish-pypi.yml` is the repo-side Trusted Publisher lane for the `brewme` package and runs behind protected environment `external-pypi-publish`; `publish-mcp-registry.yml` is the repo-side registry refresh lane for `io.github.xiaojiou176-open/brewme-mcp` and runs behind protected environment `external-mcp-registry-publish`; both still need platform-side trust/auth setup before a manual run can succeed
