...
| **GitHub social preview** | `docs/assets/brewme-social-preview.png` and tracked config entry in `config/public/github-profile.json` | tracked asset exists, but live GitHub upload still remains a manual platform step; current GitHub read-back still reports `usesCustomOpenGraphImage=false` | upload the image in the GitHub repo social preview settings | live GitHub social preview image shown on the repo |
