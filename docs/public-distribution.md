...
| **Official MCP Registry** | `starter-packs/mcp-registry/brewme-server.template.json` | metadata template exists; registry publication is not proven yet | submit against the official MCP Registry path after verifying namespace ownership and install-artifact requirements | submission receipt, registry listing URL, or namespace verification result |
