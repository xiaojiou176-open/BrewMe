...
<p>
  <img
    src="./assets/brewme-hero.svg"
    alt="BrewMe first-look preview showing the command center, digest feed, and job trace surfaces."
    width="100%"
  />
</p>
...
<p>
  <img
    src="./assets/brewme-builder-loop.svg"
    alt="BrewMe builder-facing loop showing how Codex, Claude Code, MCP clients, and operators reuse the same jobs, retrieval, and proof surfaces."
    width="100%"
  />
</p>
...
BrewMe's digest artifact template already tells the story of the output shape:
...
The honest lure is not "AI magic." It is that BrewMe already gives builders:

- a **Codex / Claude Code-friendly** MCP and HTTP API surface
- a **repo-local CLI substrate** through `./bin/brewme help` when they want one discoverable command surface
