...
This is the shortest honest OpenClaw adoption path for BrewMe.

OpenClaw is no longer limited here to a vague "generic MCP / HTTP only" story.
BrewMe now ships a first-cut local OpenClaw starter pack on top of the
...
| call BrewMe from OpenClaw-managed tools or scripts | `@brewme/sdk` | typed HTTP client over the same public contract |
| do quick command-line inspection | `@brewme/cli` | thin shell doorway into search, Ask, jobs, and templates |
| manage the full runtime in a local clone | `./bin/brewme` | repo-local operator/runtime commands stay in the checkout |
...
5. If you need a full BrewMe stack first, follow
...
- `brewme-mcp-template.json` for the MCP command template
- `skills/brewme-watchlist-briefing/SKILL.md` for the first workflow
...
```json
"cd \"/workspace/BrewMe\" && ./bin/dev-mcp"
```
...
- load `skills/brewme-watchlist-briefing/SKILL.md` as your first starter
  workflow
- use MCP when you want jobs, retrieval, artifacts, and watchlists through the
  shared agent doorway
- use `@brewme/sdk` only when you explicitly want typed code integration
...
- [starter-packs/openclaw/skills/brewme-watchlist-briefing/SKILL.md](../../starter-packs/openclaw/skills/brewme-watchlist-briefing/SKILL.md)
- [starter-packs/openclaw/brewme-mcp-template.json](../../starter-packs/openclaw/brewme-mcp-template.json)
- [templates/public-skills/openclaw/brewme-watchlist-briefing.md](../../templates/public-skills/openclaw/brewme-watchlist-briefing.md)

## Honest Boundary

- OpenClaw is now a **first-cut local starter-pack fit** through MCP + HTTP API
  plus the new public starter layer.
- BrewMe now also ships a ClawHub-oriented package metadata template, so
  the repo has a publish-ready artifact even before a live ClawHub publish receipt exists.
- This does **not** mean BrewMe ships a registry-published OpenClaw
  plugin today.
- This does **not** mean BrewMe ships an OpenClaw plugin marketplace.
