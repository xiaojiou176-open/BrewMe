...
<p align="center">
  <img
    ...
    alt="BrewMe studio preview showing source intake, digest generation, and searchable artifacts"
    width="100%"
  />
</p>
...
<p align="center">
  <img
    src="./docs/assets/brewme-hero.svg"
    alt="BrewMe preview showing the command center, digest feed, and job trace surfaces a newcomer will inspect first."
    width="100%"
  />
</p>
...
<p align="center">
  <img
    src="./docs/assets/brewme-developer-flywheel.svg"
    alt="BrewMe developer flywheel showing sources, job trace, knowledge cards, grounded search, MCP and API reuse, and the operator loop."
    width="100%"
  />
</p>
...
This is the part that makes BrewMe more than a pretty README:
...
| **Run BrewMe as an operator** | [docs/start-here.md](./docs/start-here.md), then `/subscriptions`, `/feed`, `/search`, and `/ops` after local boot | this is the shortest truthful path from clone to source-universe intake, identity-first reading flow, evidence, and triage |
| **Build on top of BrewMe** | [docs/builders.md](./docs/builders.md), [docs/mcp-quickstart.md](./docs/mcp-quickstart.md), and [docs/public-distribution.md](./docs/public-distribution.md) | these pages separate MCP, API, CLI, SDK, starter packs, and official-surface submission truth without mixing them into the newcomer path |
...
BrewMe is not just a Web app. It already has multiple access layers for builders and agent workflows:
...
| **Plugin-grade bundles and official-surface templates** | builders who want the strongest public bundles or distribution templates beyond docs-only starters | real today through `starter-packs/codex/brewme-codex-plugin/`, `starter-packs/github-copilot/brewme-github-copilot-plugin/`, `starter-packs/claude-code/brewme-claude-plugin/`, `starter-packs/vscode-agent/brewme-vscode-agent-plugin/`, `starter-packs/openclaw/clawhub.package.template.json`, `starter-packs/mcp-registry/brewme-server.template.json`, and `config/public/mcp-directory-profile.json`; readiness differs by platform |
| **Packaged public CLI** | builders who want an installable command surface before memorizing repo entrypoints | real today in [`packages/brewme-cli`](./packages/brewme-cli/README.md); it stays thinner than the repo-local runtime CLI and delegates inside a checkout |
| **Repo-local CLI substrate** | operators already inside a checkout who want the direct runtime and governance command truth | real today via `./bin/brewme help`, which remains the underlying local command truth |
...
| **Public TypeScript SDK** | TypeScript builders who want a thin client over the same HTTP contract | real today in [`packages/brewme-sdk`](./packages/brewme-sdk/README.md); it stays contract-first and builder-facing |
...
delegates to `./bin/brewme` inside a checkout, the TypeScript SDK stays a
...
| --- | --- | --- |
| **Whole BrewMe repo** | No | this repo ships Web, API, MCP, runtime, and builder layers together; it is a multi-surface product repo |
...
| **`infra/docker/brewme-api.Dockerfile` + `ghcr.io/xiaojiou176-open/brewme-api`** | dedicated product API image route | builder-facing API container lane; the GHCR package now reads back as `visibility: public`, but it still is a separate API-image builder surface rather than the default install story |
...
That is why BrewMe keeps `proof.md`, `project-status.md`, `runbook-local.md`, and the public-reference docs as separate ledgers instead of one blanket “ready” claim.
...
- **Hosted or managed BrewMe** is also a spike topic, not a current promise. Today the repository remains source-first and local-proof-first. See [docs/blueprints/2026-03-31-hosted-readiness-spike.md](./docs/blueprints/2026-03-31-hosted-readiness-spike.md).
...
| --- | --- | --- |
| discover the repo-local command surface first | `./bin/brewme help` | a thin menu over the existing `bin/*` entrypoints without inventing a second CLI stack |
...
- BrewMe is **not** described here as a turnkey hosted team workspace.
...
`postgresql+psycopg://postgres:postgres@127.0.0.1:${CORE_POSTGRES_PORT}/brewme`
...
When you stay on the repo-managed `./bin/full-stack up` path, BrewMe now
...
`brewme-local-dev-token` fallback.
...
- `brewme` from [`packages/brewme-cli`](./packages/brewme-cli/README.md) stays repo-aware and delegates to the repo-local command substrate instead of pretending to manage the whole local runtime itself
- `./bin/brewme mcp` and `./bin/brewme doctor` remain the repo-local operator entrypoints
- [`packages/brewme-sdk`](./packages/brewme-sdk/README.md) is the public TypeScript SDK surface and still stays contract-first instead of becoming a second business-logic stack
