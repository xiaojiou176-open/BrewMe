...
def _load_module():
    module_path = _repo_root() / "scripts" / "runtime" / "brewme_chrome.py"
    spec = importlib.util.spec_from_file_location("brewme_chrome", module_path)
...
def test_copy_profile_into_repo_root_rewrites_local_state_and_profile_dir(tmp_path: Path) -> None:
    ...
    assert payload["profile"]["info_cache"]["Profile 1"]["name"] == "brewme"
...
def test_list_repo_chrome_processes_filters_by_user_data_dir(
    ...
    module = _load_module()
    output = (
        "123 /Applications/Google Chrome.app/Contents/MacOS/Google Chrome --user-data-dir=/tmp/brewme/browser/chrome-user-data --profile-directory=Profile 1\n"
    ...
    rows = module.list_repo_chrome_processes(Path("/tmp/brewme/browser/chrome-user-data"))
