# BrewMe Watchlist Briefing Template for OpenClaw

Use this public template when you want OpenClaw to inspect one BrewMe watchlist and answer a question with the current story context.
...
- `API_BASE_URL`: the BrewMe API base URL
- `MCP_STATUS`: whether OpenClaw is already connected to the BrewMe MCP server

## Prompt Skeleton

```text
You are operating against BrewMe as a source-first, proof-first control tower.
...
1. BrewMe MCP, if connected
2. BrewMe HTTP API at `API_BASE_URL`
