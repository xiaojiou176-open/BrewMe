---
name: brewme_watchlist_briefing
description: Inspect one BrewMe watchlist through MCP or HTTP API and answer with grounded evidence.
---

# BrewMe Watchlist Briefing

Use this workspace skill when you want OpenClaw to inspect one BrewMe
watchlist and answer a question with the current story and evidence context.

## Before You Use This Skill

- Prefer a configured BrewMe MCP server if your OpenClaw runtime already
  has one.
- Otherwise set `SOURCE_HARBOR_API_BASE_URL` and use the BrewMe HTTP API.
- Use BrewMe web routes only as visible proof surfaces, not as hidden
  sources of truth.
